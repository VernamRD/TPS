...
bool FBatteryTests::RunTest(const FString& Parameters)
{
    ...
    const Battery BatteryDefualt;
    TestTrueExpr(FMath::IsNearlyEqual(BatteryDefualt.GetPercent(), 1.0f));
    TestTrueExpr(BatteryDefualt.GetColor() == FColor::Green);
    TestTrueExpr(BatteryDefualt.ToString().Equals("100%"));
    ...
    TestNotSame("Battery the same comparison", BatteryLow, BatteryHigh);
