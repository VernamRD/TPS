...
ATPSCharacter::ATPSCharacter()
{
    ...
    // rotates. Let that just affect the
    // camera.
    ...
    // Create a camera boom (pulls in
    // towards the player if there is a
    // collision)
    CameraBoom = CreateDefaultSubobject<USpringArmComponent>(TEXT("CameraBoom"));
    CameraBoom->SetupAttachment(RootComponent);
    CameraBoom->TargetArmLength = 300.0f;        // The camera follows
                                                 // at this distance
    ...
    // Character) are set in the derived
    // blueprint asset named MyCharacter
    // (to avoid direct content
    // references in C++)
}
...
void ATPSCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
    ...
    // rotation bindings to handle
    // different kinds of devices
    // differently "turn" handles
    // devices that provide an absolute
    // delta, such as a mouse.
    // "turnrate" is for devices that we
    // choose to treat as a rate of
    // change, such as an analog
    // joystick
...
{
// If TPS is added to a project via 'Add
// Feature' in the Unreal Editor the
// dependency on HeadMountedDisplay in
// TPS.Build.cs is not automatically
// propagated and a linker error will
// result. You will need to either:
//		Add "HeadMountedDisplay" to
//[YourProject].Build.cs
// PublicDependencyModuleNames in order
// to build successfully (appropriate if
// supporting VR).
// or:
//		Comment or delete the call to
// ResetOrientationAndPosition below
